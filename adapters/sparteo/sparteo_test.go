...
func TestMakeRequests_Valid(t *testing.T) {
	...
	// Create a valid impression with a banner and proper bidder extension (networkId and tagid).
	imp := openrtb2.Imp{
		ID: "imp1",
	...
	if len(requests) == 1 {
		...
		sparteoMap, ok := impExtMap["sparteo"].(map[string]interface{})
		require.True(t, ok, "Expected 'sparteo' object in imp.Ext")

		// Check that the params include the adUnitCode (which should come from tagid).
		paramsMap, ok := sparteoMap["params"].(map[string]interface{})
		require.True(t, ok, "Expected 'params' object in imp.Ext.sparteo")
		assert.Equal(t, "some-adunit-code", paramsMap["adUnitCode"], "Expected adUnitCode (tagid) to be set")
...
// TestMakeRequests_MissingTagID ensures that MakeRequests returns an error when the required 'tagid' is missing in the bidder extension.
func TestMakeRequests_MissingTagID(t *testing.T) {
	bidder, _ := Builder(openrtb_ext.BidderName("sparteo"), config.Adapter{}, config.Server{})

	imp := openrtb2.Imp{
		ID:     "imp-without-tagid",
	...
	require.NotEmpty(t, errs, "Expected an error due to missing tagid")
	assert.Contains(t, errs[0].Error(), "Missing tagid for imp ID: imp-without-tagid")
}
...
// TestMakeRequests_MixedMissingParamsImps tests the scenario where the bid request contains both valid and invalid (missing tagid) impressions.
// The adapter should merge valid impressions and return errors only for the invalid ones.
func TestMakeRequests_MixedMissingParamsImps(t *testing.T) {
	bidder, _ := Builder(openrtb_ext.BidderName("sparteo"), config.Adapter{Endpoint: "http://fake.endpoint"}, config.Server{})

	impValid := openrtb2.Imp{
		ID:     "imp-valid",
		Banner: &openrtb2.Banner{W: int64Ptr(300), H: int64Ptr(250)},
		Ext: json.RawMessage(`{
            "bidder": {
	...
	impMissingTagID := openrtb2.Imp{
		ID:  "imp-missing-tag",
		Ext: json.RawMessage(`{"bidder": {"networkId": "netY"}}`),
	}

	req := &openrtb2.BidRequest{
		Imp: []openrtb2.Imp{impValid, impMissingTagID},
		Site: &openrtb2.Site{
			Domain: "dev.sparteo.com",
			Publisher: &openrtb2.Publisher{
				Domain: "dev.sparteo.com",
	...
	// Execute MakeRequests. We expect one valid request (merging imps) and one error due to missing tagid.
	requests, errs := bidder.MakeRequests(req, &adapters.ExtraRequestInfo{})

	require.Len(t, errs, 1, "Expected one error for the missing tagid")
	assert.Contains(t, errs[0].Error(), "Missing tagid for imp ID: imp-missing-tag")

	require.Len(t, requests, 1, "Expected a single requestData, as the adapter merges all imps")

	var updatedReq openrtb2.BidRequest
	err := json.Unmarshal(requests[0].Body, &updatedReq)
	require.NoError(t, err, "Failed to unmarshal request body")

	// Verify that the valid impression's extension was updated with proper parameters.
	var extMap map[string]interface{}
	err = json.Unmarshal(updatedReq.Imp[0].Ext, &extMap)
	require.NoError(t, err, "Failed to unmarshal updated ext for the valid imp")

	sparteoMap, ok := extMap["sparteo"].(map[string]interface{})
	require.True(t, ok, "Expected 'sparteo' object in first imp.Ext")

	paramsMap, ok := sparteoMap["params"].(map[string]interface{})
	require.True(t, ok, "Expected 'params' object in first imp.Ext.sparteo")
	assert.Equal(t, "tag123", paramsMap["adUnitCode"], "Expected adUnitCode from the valid imp")

	// Verify that the site publisher's extension has been updated with the networkId from the valid imp.
	require.NotNil(t, updatedReq.Site, "Expected site to be non-nil")
	require.NotNil(t, updatedReq.Site.Publisher, "Expected publisher to be non-nil")
	require.NotNil(t, updatedReq.Site.Publisher.Ext, "Expected publisher ext to be set")
	...
	pubParams, ok := pubExt["params"].(map[string]interface{})
	require.True(t, ok, "Expected 'params' object in publisher ext")
	assert.Equal(t, "netX", pubParams["networkId"], "Expected networkId from the valid imp to be set in publisher ext")
}
...
func TestParseExt_Success(t *testing.T) {
	imp := openrtb2.Imp{
		ID:  "imp1",
		Ext: json.RawMessage(`{"bidder":{"networkId":"netABC", "tagid": "my-tag"}}`),
	}
	res, err := parseExt(&imp)
	require.NoError(t, err)
	require.NotNil(t, res)
	assert.Equal(t, "netABC", res.NetworkId)
	assert.Equal(t, "my-tag", res.TagId)
}
...
func TestGetNetworkId_HasNetworkId(t *testing.T) {
	imp := openrtb2.Imp{
		ID:  "impWithNetwork",
		Ext: json.RawMessage(`{"bidder": {"networkId": "netXYZ", "tagid": "some-tag"}}`),
	}
...
func TestGetNetworkId_MissingNetworkId(t *testing.T) {
	imp := openrtb2.Imp{
		...
		Ext: json.RawMessage(`{"bidder": {"tagid": "some-tag"}}`),
	}
	extImp, err := parseExt(&imp)
	require.NoError(t, err)
...
// TestGetTagId_HasTagID verifies that tagid is correctly parsed when provided.
func TestGetTagId_HasTagID(t *testing.T) {
	imp := openrtb2.Imp{
		ID:  "impWithTag",
		Ext: json.RawMessage(`{"bidder": {"networkId": "netABC", "tagid": "my-tag"}}`),
	}
	extImp, err := parseExt(&imp)
	require.NoError(t, err)
	assert.Equal(t, "my-tag", extImp.TagId)
}

// TestGetTagId_MissingTagID ensures that if tagid is missing from the bidder extension,
// the parsed result is empty.
func TestGetTagId_MissingTagID(t *testing.T) {
	imp := openrtb2.Imp{
		ID:  "impWithoutTag",
	...
	extImp, err := parseExt(&imp)
	require.NoError(t, err)
	assert.Empty(t, extImp.TagId, "Expected tagid to be empty when missing")
}
...
func TestMakeRequests_MergeBidderParams(t *testing.T) {
	...
	assert.Equal(t, "my-tag", paramsMap["adUnitCode"], "Expected adUnitCode to be set from tagid")
